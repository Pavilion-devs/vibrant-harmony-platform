...
const PageHeader = ({ title, subtitle, image, children, className }: PageHeaderProps) => {
  return (
    ...
    >
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-r from-vara-primary/90 via-vara-primary/70 to-vara-primary/50 z-0" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent z-0" />
      
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 relative z-10 text-white">
      ...
      {/* Decorative elements */}
      <div className="absolute bottom-8 left-8 w-16 h-16 border-l-4 border-b-4 border-white/20 hidden md:block"></div>
      <div className="absolute top-1/3 right-8 w-20 h-20 border-r-4 border-t-4 border-vara-secondary/30 hidden lg:block"></div>
    </div>
  );
};
